...
export class F18Animation {
     ...
     */
    private lightFlash() {
        //Since the material is shared, it cannot be called multiple times.
        if (lightClear) {
            return
        }
    ...
    public undercarriageChange() {
        ...
        //Toggle freeze time 1500 milliseconds
    ...
    /**
     * Update aircraft tail smoke
     */
    private updateFlames() {
        //If the thrust of the aircraft is greater than 250, it will emit tail smoke.
    ...
    public render() {
        //If the speed is high enough, stretching the aircraft wings creates a compressed air flow.
    ...
    private updateUndercarriage() {
        //If the landing gear is open, update the landing gear and tire bones
        if (this.undercarriageState) {
            //Get the suspension length of the current tire
